...
if command -v python > /dev/null
    set -q _python_ver; or set -Ux _python_ver (string match -r '\d+.\d+[.\d]*' (command python -V 2>&1))
end
function _set_python_venv_ver --on-variable VIRTUAL_ENV  # {{{
    ...
    case 'ERASE'
        set -e _python_venv
        set -Ux _python_ver (string match -r '\d+.\d+[.\d]*' (command python -V 2>&1))
    case 'SET'
...
if command -v go > /dev/null
    set -q _go_ver; or set -Ux _go_ver (string match -r '\d+.\d+[.\d]*' (command go version 2>&1))
end
function _set_go_ver --on-variable GOROOT  # {{{
    command -v python > /dev/null; or return
    switch $argv[2]
    case 'ERASE'
        set -Ux _go_ver (string match -r '\d+.\d+[.\d]*' (command go version 2>&1))
    case 'SET'
